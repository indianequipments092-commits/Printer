...
class PdfReaderActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        // No bottom page arrows. Scroll vertically and use the page-number rail.
        setContentView(root)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 9001 && resultCode == RESULT_OK && data?.data != null) {
            try { data.data?.let { contentResolver.takePersistableUriPermission(it, Intent.FLAG_GRANT_READ_URI_PERMISSION) } } catch (_: Throwable) { }
            openUri(data.data!!)
        }
    }
                ...
                for (index in 0 until r.pageCount) {
                    ...
                    runOnUiThread {
                        ...
                        if (index == r.pageCount - 1) {
                            loading.visibility = View.GONE
                            pageScroll.post { setZoom(1f); updatePageLabel() }
                        }
                    }
                }
            } catch (_: Throwable) {
    ...
    private fun scrollToPage(index: Int) {
        ...
        pageScroll.post { pageScroll.smoothScrollTo(0, max(0, pageViews[index].top - dp(8))) }
