...
class PdfReaderActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        // Use a plain FrameLayout here instead of ScrollView/FrameLayout gravity APIs.
        // This avoids the Kotlin Android synthetic/property resolution issue seen in CI.
        ...
        val imageParams = FrameLayout.LayoutParams(-1, -2)
        viewer.addView(pageImage, imageParams)

        loading = ProgressBar(this).apply { visibility = View.GONE }
        val loadingParams = FrameLayout.LayoutParams(dp(48), dp(48))
        loadingParams.leftMargin = dp(8)
        loadingParams.topMargin = dp(8)
        viewer.addView(loading, loadingParams)
        ...
        val pageRow = LinearLayout(this).apply { gravity = android.view.Gravity.CENTER }
        pageRow.addView(button("‹", 52) { showPage(currentPage - 1) })
        pageRow.addView(button("PAGE", 64) { showPageJump() })
        pageRow.addView(button("›", 52) { showPage(currentPage + 1) })
        controls.addView(pageRow)

        val zoomRow = LinearLayout(this).apply { gravity = android.view.Gravity.CENTER }
        zoomRow.addView(button("−", 52) { setZoom(zoom - 0.1f) })
        zoomRow.addView(button("FIT", 64) { setZoom(1f) })
        zoomRow.addView(button("+", 52) { setZoom(zoom + 0.1f) })
        zoomRow.addView(button("↻", 52) {
            rotation += 90f
    ...
    private fun renderCurrentPage() {
        ...
        if (currentPage !in 0 until r.pageCount) return
        loading.visibility = View.VISIBLE
        executor.execute {
            try {
                r.openPage(currentPage).use { page ->
                    ...
                    runOnUiThread {
                        ...
                        pageLabel.text = "Page ${currentPage + 1} / ${r.pageCount}"
    ...
    private fun showPageJump() {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("GO") { _, _ -> input.text.toString().toIntOrNull()?.let { showPage(it - 1) } }
            .show()
    }

    private fun setZoom(value: Float) {
        zoom = value.coerceIn(0.5f, 2.5f)
        pageImage.scaleX = zoom
        pageImage.scaleY = zoom
        zoomLabel.text = "${(zoom * 100).toInt()}%"
    }
