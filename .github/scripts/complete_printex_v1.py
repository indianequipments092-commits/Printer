        ...
        AlertDialog.Builder(this).setTitle("Pages").setItems(values) { _, which ->
            ...
            if (selected != "Page range") {
                ...
                val input = EditText(this).apply {
                    hint = "Example: 1,3,5 or 1-5 or 1,3-5,8"
                    setSingleLine(true)
                    setText(printexPageRange)
                }
...
if 'private fun printexPageIndices(' not in s:
    ...
    private fun printexApplyAdjustments(src: Bitmap): Bitmap {
    ...
    private fun printexSourceBitmaps(): List<Bitmap> {
        val file = printexFile ?: return emptyList()
        ...
        val selected = when {
            printexPages == "Odd pages" -> (0 until renderer.pageCount).filter { it % 2 == 0 }
            printexPages == "Even pages" -> (0 until renderer.pageCount).filter { it % 2 == 1 }
            printexPages == "Page range" -> printexPageIndices(printexPageRange, renderer.pageCount)
            else -> (0 until renderer.pageCount).toList()
        }
    ...
    private fun printexPrintPdf(): File {
        val source = printexSourceBitmaps()
        if (source.isEmpty()) throw IllegalStateException("No printable document selected")
            ...
            val adapter = object : android.print.PrintDocumentAdapter() {
                ...
                override fun onWrite(pages: Array<android.print.PageRange>, destination: android.os.ParcelFileDescriptor, cancellationSignal: android.os.CancellationSignal, callback: android.print.PrintDocumentAdapter.WriteResultCallback) {
                    try {
                        ...
                        val file = prepared ?: throw IllegalStateException("Print document was not prepared")
                        java.io.FileInputStream(file).use { input -> FileOutputStream(destination.fileDescriptor).use { output -> input.copyTo(output) } }
