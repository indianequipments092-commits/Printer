...
# Brightness and contrast are user-facing values centered at 0. Internally contrast 0 = neutral (1.0x).
...
s = s.replace('slider("Contrast", 50, 200, 100) { contrast = it / 100f; updatePreview() }', 'slider("Contrast", -100, 100, settingPrefs.getInt("current_contrast", 0)) { contrast = 1f + it / 100f; settingPrefs.edit().putInt("current_contrast", it).apply(); updatePreview() }')
...
# Add Default / Save Setting / Saved Settings controls.
...
# Insert persistent settings helpers.
...
# Make the current brightness/contrast settings affect the actual scanner result.
