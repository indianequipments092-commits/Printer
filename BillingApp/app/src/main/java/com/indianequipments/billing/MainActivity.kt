...
class MainActivity : Activity() {
    ...
    private fun navigate(name: String, action: () -> Unit) {
        if (currentScreen != name) {
            ...
            screenStack.addLast { if (old == "home") showHome(false) else when (old) {
    ...
    private fun root(title: String, subtitle: String? = null, back: Boolean = true): LinearLayout {
        ...
        titles.addView(label(title, 21, text, true))
        if (subtitle != null) titles.addView(label(subtitle, 12, muted, false))
        top.addView(titles, LinearLayout.LayoutParams(0, 58, 1f))
        top.addView(iconButton("⋮") {}, LinearLayout.LayoutParams(50, 52))
        root.addView(top)
        setContentView(root)
        return root
    }
    ...
    private fun label(s: String, size: Float, color: Int = text, bold: Boolean = false) = TextView(this).apply {
        this.text = s; textSize = size; setTextColor(color); setPadding(0, 3, 0, 3)
        if (bold) typeface = Typeface.DEFAULT_BOLD
    }

    private fun iconButton(symbol: String, click: () -> Unit) = Button(this).apply {
        text = symbol; textSize = 24f; setTextColor(text); setBackgroundColor(Color.TRANSPARENT); setOnClickListener { click() }
    }

    private fun card(title: String, subtitle: String, icon: String, color: Int, click: () -> Unit): LinearLayout {
        val c = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(16, 14, 12, 14); setBackgroundColor(surface)
        ...
        tx.addView(label(title, 16, text, true)); tx.addView(label(subtitle, 12, muted))
        c.addView(tx, LinearLayout.LayoutParams(0, 70, 1f)); c.addView(label("›", 30, muted))
        val lp = LinearLayout.LayoutParams(-1, 78); lp.setMargins(0, 6, 0, 6); c.layoutParams = lp
        return c
    }
    ...
    private fun button(textValue: String, click: () -> Unit) = Button(this).apply {
        text = textValue; textSize = 14f; setTextColor(text); setBackgroundColor(surface2); setOnClickListener { click() }
    }

    private fun input(hint: String) = EditText(this).apply {
        this.hint = hint; hintTextColor = muted; setTextColor(text); setSingleLine(true); setPadding(14, 12, 14, 12); setBackgroundColor(surface2)
    }

    private fun showHome(push: Boolean = true) {
        if (push) navigate("home") { showHome(false) } else { currentScreen = "home"; screenStack.clear() }
        refresh(); val v = root("INDIAN EQUIPMENTS", "GST Billing Studio • FY ${Numbering.financialYear()}", false)
        ...
        hero.addView(label("Good Morning, Admin 👋", 22, Color.WHITE, true))
        hero.addView(label("Premium GST Billing Studio", 13, Color.rgb(207, 225, 255)))
        hero.addView(label("Template-driven • Tally-style workflow", 12, Color.rgb(180, 210, 255)))
        val stats = LinearLayout(this).apply { setPadding(0, 14, 0, 0) }
        stats.addView(stat("FY", Numbering.financialYear()), LinearLayout.LayoutParams(0, 55, 1f))
        stats.addView(stat("Customers", customers.size.toString()), LinearLayout.LayoutParams(0, 55, 1f))
        stats.addView(stat("Items", items.size.toString()), LinearLayout.LayoutParams(0, 55, 1f))
        hero.addView(stats); v.addView(hero)
        v.addView(label("QUICK CREATE", 12, muted, true))
        val grid = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
        grid.addView(actionCard("＋ Invoice", "GST Tax Invoice", blue) { newBill() }, LinearLayout.LayoutParams(0, 82, 1f).also { it.setMargins(0, 5, 5, 5) })
        grid.addView(actionCard("▣ Quotation", "Sales quotation", green) { newBill("Quotation") }, LinearLayout.LayoutParams(0, 82, 1f).also { it.setMargins(5, 5, 0, 5) })
        v.addView(grid)
        val grid2 = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
        grid2.addView(actionCard("▤ Challan", "Delivery challan", orange) { newBill("Delivery Challan") }, LinearLayout.LayoutParams(0, 82, 1f).also { it.setMargins(0, 0, 5, 5) })
        grid2.addView(actionCard("▥ Proforma", "Proforma invoice", purple) { newBill("Proforma Invoice") }, LinearLayout.LayoutParams(0, 82, 1f).also { it.setMargins(5, 0, 0, 5) })
        v.addView(grid2)
    ...
    private fun customerMaster(push: Boolean = true) {
        ...
        refresh(); val v = root("Customers / Party Master", "${customers.size} saved parties")
        val search = input("Search by name, GSTIN, phone or city…"); v.addView(search)
        v.addView(button("＋  Add New Customer") { customerDialog() })
        customers.forEach { c -> v.addView(card(c.name, "${c.gstin.ifBlank { "GSTIN not set" }} • ${c.state}", "👤", green) { customerDialog(c) }) }
    }
    ...
    private fun customerDialog(existing: Customer? = null) {
        ...
        fields.forEach { f -> val lp = LinearLayout.LayoutParams(-1, 52); lp.setMargins(0, 4, 0, 4); box.addView(f, lp) }
        AlertDialog.Builder(this).setTitle(if (existing == null) "Add Customer" else "Edit Customer").setView(box)
            .setPositiveButton("Save") { _, _ -> if (existing == null) db.addCustomer(Customer(0, fields[0].text.toString(), fields[1].text.toString(), fields[2].text.toString(), fields[3].text.toString(), fields[4].text.toString(), fields[5].text.toString(), fields[6].text.toString())); refresh(); customerMaster(false) }
            .setNegativeButton("Cancel", null).show()
    }

    private fun itemMaster(push: Boolean = true) {
        if (push) navigate("items") { itemMaster(false); return }
        refresh(); val v = root("Item / Product Master", "HSN • description • unit • GST • rate")
        v.addView(button("＋  Add New Item") { itemDialog() })
        items.forEach { it -> v.addView(card(it.name, "HSN ${it.hsn} • ${it.unit} • GST ${it.gst}% • ₹${it.defaultRate}", "📦", cyan) { itemDialog(it) }) }
    }
    ...
    private fun itemDialog(existing: Item? = null) {
        ...
        AlertDialog.Builder(this).setTitle(if (existing == null) "Add Item" else "Edit Item").setView(box)
            .setPositiveButton("Save") { _, _ -> if (existing == null) db.addItem(Item(0, fields[0].text.toString(), fields[1].text.toString(), fields[2].text.toString(), fields[3].text.toString().ifBlank { "Nos" }, fields[4].text.toString().toDoubleOrNull() ?: 0.0, fields[5].text.toString().toDoubleOrNull() ?: 0.0)); refresh(); itemMaster(false) }
            .setNegativeButton("Cancel", null).show()
    }

    private fun newBill(preset: String? = null) {
        navigate("newbill") { newBill(preset) ; return }
        refresh(); lines.clear(); selectedCustomer = null
        val v = root("New Document", "Step 1 of 3 • Customer & document details")
        val type = Spinner(this); type.adapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, arrayOf("Invoice", "Delivery Challan", "Proforma Invoice", "Quotation")); preset?.let { val a = listOf("Invoice", "Delivery Challan", "Proforma Invoice", "Quotation"); type.setSelection(a.indexOf(it).coerceAtLeast(0)) }
        v.addView(label("DOCUMENT TYPE", 11, muted, true)); v.addView(type)
        val party = Spinner(this); party.adapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, customers.map { it.name }.ifEmpty { listOf("No customers — add one first") })
        v.addView(label("CUSTOMER / PARTY", 11, muted, true)); v.addView(party)
        val tax = Spinner(this); tax.adapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, arrayOf("CGST + SGST", "IGST", "No Tax"))
        v.addView(label("TAX MODE", 11, muted, true)); v.addView(tax)
        ...
        v.addView(button("Next  →  Review & Generate") {
            if (customers.isEmpty() || items.isEmpty() || lines.isEmpty() || party.selectedItemPosition < 0) { toast("Select customer and add at least one item") } else {
                selectedCustomer = customers[party.selectedItemPosition]
                val document = type.selectedItem.toString(); val number = if (document == "Invoice") Numbering.next(this) else document.uppercase(Locale.US).replace(" ", "-") + "/" + Numbering.financialYear() + "/" + System.currentTimeMillis().toString().takeLast(5)
                val bill = Bill(0, document, number, Numbering.date(), selectedCustomer!!, lines.toList(), if (tax.selectedItemPosition == 1) "IGST" else if (tax.selectedItemPosition == 2) "NONE" else "CGST_SGST", System.currentTimeMillis())
                db.saveBill(bill.type, bill.number, bill.date, bill.customer.id, bill.taxMode, bill.lines); generateFiles(bill)
            }
        })
    }
    ...
    private fun lineDialog(parent: LinearLayout) {
        val box = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(8, 0, 8, 0) }
        val sp = Spinner(this); sp.adapter = ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, items.map { it.name })
        val desc = Spinner(this); val qty = input("Quantity"); val rate = input("Rate"); val discount = input("Discount")
        sp.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            ...
            override fun onItemSelected(p: AdapterView<*>?, view: View?, pos: Int, id: Long) {
                ...
                val choices = item.description.split('|').map { it.trim() }.filter { it.isNotBlank() }.ifEmpty { listOf(item.name) }
                desc.adapter = ArrayAdapter(this@MainActivity, android.R.layout.simple_spinner_dropdown_item, choices); rate.setText(item.defaultRate.toString())
            }
        }
    ...
    private fun history(push: Boolean = true) {
        if (push) navigate("history") { history(false); return }; val v = root("Bills & History", "All documents • newest first"); v.addView(input("Search invoice no., customer or type…")); db.recentBills().forEach { r -> v.addView(card(r[0].toString(), "${r[1]} • ${r[2]} • ${r[3]} • Taxable ₹${r[4]}", "🧾", purple) {}) }
    }

    private fun report(push: Boolean = true) {
        if (push) navigate("reports") { report(false); return }; val v = root("Reports / Summary", "Business overview • FY ${Numbering.financialYear()}"); val b = db.recentBills(); val total = b.sumOf { it[4].toDoubleOrNull() ?: 0.0 }; v.addView(card("Total Bills", b.size.toString(), "🧾", blue) {}); v.addView(card("Taxable Value", "₹${String.format(Locale.US, "%.2f", total)}", "₹", green) {}); v.addView(card("Customers", customers.size.toString(), "👥", cyan) {}); v.addView(card("Items", items.size.toString(), "📦", orange) {})
    }

    private fun settings(push: Boolean = true) {
        if (push) navigate("settings") { settings(false); return }; val v = root("Settings / Excel Template", "Company, numbering and output controls"); v.addView(card("Excel Template", if (templateFile.exists()) "Imported ✓ • master format ready" else "Not imported yet", "▣", green) { pickTemplate() }); v.addView(card("Invoice Numbering", "IE/${Numbering.financialYear()}/16 • automatic sequence", "#", blue) { val e = input("Next invoice number"); e.setText("16"); AlertDialog.Builder(this).setTitle("Invoice numbering").setView(e).setPositiveButton("Save") { _, _ -> Numbering.setInitialSequence(this, e.text.toString().toIntOrNull() ?: 16); toast("Next sequence saved") }.setNegativeButton("Cancel", null).show() }); v.addView(label("OUTPUT", 11, muted, true)); v.addView(button("Import / Replace Excel Invoice Template") { pickTemplate() }); v.addView(label("The workbook stays the master visual format. The app fills its cells; it does not redesign your bill.", 12, muted))
    }
