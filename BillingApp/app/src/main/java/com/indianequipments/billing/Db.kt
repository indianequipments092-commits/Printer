...
class BillingDb(context: Context) : SQLiteOpenHelper(context, "billing.db", null, 1) {
    ...
    fun saveBill(type: String, number: String, date: String, customerId: Long, taxMode: String, lines: List<BillLine>): Long { val db = writableDatabase; db.beginTransaction(); return try { val id = db.insert("bills", null, android.content.ContentValues().apply { put("type", type); put("number", number); put("date", date); put("customer_id", customerId); put("tax_mode", taxMode); put("created_at", System.currentTimeMillis()) }); lines.forEach { l -> db.insert("bill_lines", null, android.content.ContentValues().apply { put("bill_id", id); put("item_id", l.item.id); put("description", l.description); put("qty", l.qty); put("rate", l.rate); put("discount", l.discount) }) }; db.setTransactionSuccessful(); id } finally { db.endTransaction() } }
