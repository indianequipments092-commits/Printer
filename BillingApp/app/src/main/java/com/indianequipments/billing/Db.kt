...
data class BillLine(val item: Item, var description: String, var qty: Double, var rate: Double, var discount: Double = 0.0) {
    val taxable: Double get() = (qty * rate - discount).coerceAtLeast(0.0)
}

data class Bill(val id: Long, val type: String, val number: String, val date: String, val customer: Customer, val lines: List<BillLine>, val taxMode: String, val createdAt: Long)
...
class BillingDb(context: Context) : SQLiteOpenHelper(context, "billing.db", null, 1) {
    ...
    fun addCustomer(c: Customer): Long = writableDatabase.insert("customers", null, android.content.ContentValues().apply { put("name",c.name);put("gstin",c.gstin);put("address",c.address);put("state",c.state);put("state_code",c.stateCode);put("phone",c.phone);put("email",c.email) })
    ...
    fun addItem(i: Item): Long = writableDatabase.insert("items", null, android.content.ContentValues().apply { put("name",i.name);put("description",i.description);put("hsn",i.hsn);put("unit",i.unit);put("gst",i.gst);put("rate",i.defaultRate) })
    ...
    fun saveBill(type:String, number:String, date:String, customerId:Long, taxMode:String, lines:List<BillLine>):Long {
        val db=writableDatabase; db.beginTransaction(); try { val id=db.insert("bills",null,android.content.ContentValues().apply{put("type",type);put("number",number);put("date",date);put("customer_id",customerId);put("tax_mode",taxMode);put("created_at",System.currentTimeMillis())}); lines.forEach{ l->db.insert("bill_lines",null,android.content.ContentValues().apply{put("bill_id",id);put("item_id",l.item.id);put("description",l.description);put("qty",l.qty);put("rate",l.rate);put("discount",l.discount)})}; db.setTransactionSuccessful(); return id } finally { db.endTransaction() }
    }
    fun recentBills(): List<Array<String>> = readableDatabase.rawQuery("SELECT b.number,b.type,b.date,c.name,(SELECT COALESCE(SUM((qty*rate)-discount),0) FROM bill_lines bl WHERE bl.bill_id=b.id) FROM bills b LEFT JOIN customers c ON c.id=b.customer_id ORDER BY b.id DESC LIMIT 100",null).use{cur->buildList{while(cur.moveToNext()) add(arrayOf(cur.getString(0),cur.getString(1),cur.getString(2),cur.getString(3)? : "",String.format("%.2f",cur.getDouble(4))))}}
}
