...
class BillingDb(context: Context) : SQLiteOpenHelper(context, "billing.db", null, 2) {
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("CREATE TABLE bills(id INTEGER PRIMARY KEY AUTOINCREMENT,type TEXT,number TEXT,date TEXT,customer_id INTEGER,tax_mode TEXT,created_at INTEGER,delivery_note TEXT,destination TEXT)")
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) {
            db.execSQL("ALTER TABLE bills ADD COLUMN delivery_note TEXT DEFAULT ''")
            db.execSQL("ALTER TABLE bills ADD COLUMN destination TEXT DEFAULT ''")
        }
    }
    ...
    fun customers(): List<Customer> = readableDatabase.rawQuery("SELECT * FROM customers ORDER BY name COLLATE NOCASE", null).use { cur ->
        buildList {
            while (cur.moveToNext()) add(Customer(cur.getLong(0), cur.getString(1), cur.getString(2) ?: "", cur.getString(3) ?: "", cur.getString(4) ?: "", cur.getString(5) ?: "", cur.getString(6) ?: "", cur.getString(7) ?: ""))
        }
    }
    ...
    fun saveBill(type: String, number: String, date: String, customerId: Long, taxMode: String, lines: List<BillLine>, deliveryNote: String = "", destination: String = ""): Long {
        ...
        db.beginTransaction()
        return try {
            val id = db.insert("bills", null, android.content.ContentValues().apply {
                put("type", type); put("number", number); put("date", date); put("customer_id", customerId); put("tax_mode", taxMode); put("created_at", System.currentTimeMillis()); put("delivery_note", deliveryNote); put("destination", destination)
            })
            lines.forEach { l -> db.insert("bill_lines", null, android.content.ContentValues().apply {
                put("bill_id", id); put("item_id", l.item.id); put("description", l.description); put("qty", l.qty); put("rate", l.rate); put("discount", l.discount)
            }) }
    ...
    fun deleteBill(number: String): Int {
        val db = writableDatabase
        db.beginTransaction()
        return try {
            val ids = mutableListOf<Long>()
            db.rawQuery("SELECT id FROM bills WHERE number=?", arrayOf(number)).use { cur -> while (cur.moveToNext()) ids.add(cur.getLong(0)) }
            ids.forEach { db.delete("bill_lines", "bill_id=?", arrayOf(it.toString())) }
            val deleted = db.delete("bills", "number=?", arrayOf(number))
            db.setTransactionSuccessful()
            deleted
        } finally { db.endTransaction() }
    }

    fun recentBills(): List<Array<String>> = readableDatabase.rawQuery("SELECT b.id,b.number,b.type,b.date,c.name,(SELECT COALESCE(SUM((qty*rate)-discount),0) FROM bill_lines bl WHERE bl.bill_id=b.id) FROM bills b LEFT JOIN customers c ON c.id=b.customer_id ORDER BY b.id DESC LIMIT 100", null).use { cur ->
        buildList {
            while (cur.moveToNext()) add(arrayOf(cur.getString(0), cur.getString(1), cur.getString(2), cur.getString(3), cur.getString(4) ?: "", String.format(java.util.Locale.US, "%.2f", cur.getDouble(5))))
        }
    }
}
